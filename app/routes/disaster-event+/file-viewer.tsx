...
export const loader = authLoaderPublicOrWithPerm("ViewData", async ({ request, userSession }: any) => {
  ...
  if (fileId && effectiveUserSession.countryAccountsId) {
    try {
      ...
      const event = await dr
        .select({ id: disasterEventTable.id, countryAccountsId: disasterEventTable.countryAccountsId })
      ...
      // If event found but doesn't belong to user's tenant
        ...
        console.warn(`Access denied: User from tenant ${effectiveUserSession.countryAccountsId} attempted to access file for disaster event ${fileId} belonging to tenant ${event[0].countryAccountsId}`);
        // Redirect to unauthorized page with specific reason
        const unauthorizedUrl = `/error/unauthorized?reason=access-denied`;
        return new Response(null, {
          status: 302,
  ...
  return await handleFileRequest(request, DISASTER_EVENT_UPLOAD_PATH, download, effectiveUserSession);
});