...
export const action = authLoaderWithPerm("ViewData", async (args: LoaderFunctionArgs) => {
  ...
  if (!userSession) {
    console.error("No user session available in hazardous-event+/file-pre-upload action");
    throw new Response("Unauthorized", { status: 401 });
  }

  const tenantContext = await getTenantContext(userSession);

  console.log("hazardous-event+/file-pre-upload action called with user session:", !!userSession);
