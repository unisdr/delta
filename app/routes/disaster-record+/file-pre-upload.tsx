...
export const action = authLoaderWithPerm("ViewData", async (args: LoaderFunctionArgs) => {
  ...
  if (!userSession) {
    console.error("No user session available in disaster-record+/file-pre-upload action");
    throw new Response("Unauthorized", { status: 401 });
  }

  const tenantContext = await getTenantContext(userSession);

  console.log("disaster-record+/file-pre-upload action called with user session:", !!userSession);
