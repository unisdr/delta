 ...
 */
export const calculateFaoAgriculturalDamage = async (table: any, subsector: FaoAgriSubsector): Promise<FaoAgriculturalDamage> => {
    type AssetDamages = {
        machinery: string;
        equipment: string;
        facilities: string;
 ...
 */
export const calculateFaoAgriculturalLoss = async (table: any, subsector: FaoAgriSubsector): Promise<FaoAgriculturalLoss> => {
    ...
    return {
        production: {
            ...
            livestockProductivity: subsector === 'livestock' ? Number(productionResult.production_loss) : 0,
