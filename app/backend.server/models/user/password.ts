...
export function generatePassword(length: number): string {
    ...
    const allGroups = [upper, lower, digits, special];

    // Shuffle array
    const shuffle = <T>(array: T[]): T[] =>
      array.sort(() => Math.random() - 0.5);

    // Ensure at least 2 of the 4 groups are present
    const selectedGroups = shuffle(allGroups).slice(0, 2);

    // Add more groups to diversify
    while (selectedGroups.length < 4) {
      if (Math.random() < 0.5) break;
      const remaining = allGroups.filter(g => !selectedGroups.includes(g));
      if (remaining.length > 0) selectedGroups.push(remaining[0]);
    }
    ...
    for (const group of selectedGroups) {
      passwordChars.push(group[Math.floor(Math.random() * group.length)]);
    }

    // Fill the rest of the password with random characters from all selected groups
    const allSelectedChars = selectedGroups.join("");
    while (passwordChars.length < length) {
      const char = allSelectedChars[Math.floor(Math.random() * allSelectedChars.length)];
