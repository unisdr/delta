...
export async function deleteAll(countryAccountsId: string) {
	return await dr.transaction(async (tx: Tx) => {
		try {
			...
			logger.error('Failed to delete divisions by country account ID', { error, countryAccountsId });
			throw new DatabaseError('Failed to delete divisions by country account ID', { cause: error });
		}
	});
}
