...
describe('DataManager', () => {
	...
	it('initializes with no updates', () => {
		...
		manager.init(cols, initialData, ids)
	...
	it('initializes with updates', () => {
		...
		const manager = new DataManager()
		manager.init(cols, initialData, ids, previousUpdates)
		let res = manager.applyUpdates()
		assert.deepEqual(res, [
			[
	...
	it('check totals - basic', () => {
		...
		let res = manager.groupTotalsMatch()
		let want = new Map([
			["1", [false]],
		])
