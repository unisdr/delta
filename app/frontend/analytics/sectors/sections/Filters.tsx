...
const Filters: React.FC<FiltersProps> = ({
  ...
  const handleFilterChange = async (field: string, value: string) => {
    ...
    // Dynamic filtering for disaster events based on sectorId
    if (field === "sectorId" && value) {
      try {
        setLoading(true);
        const filteredEvents = await fetch(
          `/api/disaster-events?sectorId=${value}`
        ).then((res) => res.json());
        setDisasterEvents(filteredEvents);
      } catch (error) {
        console.error("Error fetching filtered disaster events:", error);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="mg-grid mg-grid__col-3">
            ...
            if (e.key === "Enter" && filters.disasterEventId.trim() !== "") {
              try {
                ...
                const searchResults = await fetch(
                  `/api/disaster-events?query=${encodeURIComponent(
      ...
      >
        <button
          ...
          type="button"
          onClick={() =>
            setFilters({ sectorId: "", disasterEventId: "", dateRange: "" })
          }
        >
          Clear
        </button>
